    ...
    //output full document 
        ...
        try{
            ...
            var docs = await aisdk_20201118.aggregate([
                {$match: {MMSI:mmsi}},
                {$project: {_id:0,Latitude:{ $arrayElemAt: [ '$Position.coordinates', 0 ] },Longitude:{ $arrayElemAt: [ '$Position.coordinates', 1 ]}}}]).toArray();
            return docs[0];        
        }catch (error){
            return error
        }finally{
...
module.exports = {find,deleteOldMessages, findAllRecentPositions, findPortByName, isStub }
